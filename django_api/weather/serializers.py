...
class WeatherRequestSerializer(serializers.Serializer):
    ...
    area_code = serializers.CharField(
        required=True,
        help_text="予報区コード（6桁）。例: 130010=東京地方, 270000=大阪府",
    )
...
class WeatherResponseSerializer(serializers.Serializer):
    ...
    temp_min = serializers.IntegerField(
        read_only=True,
        allow_null=True,
        help_text="最低気温（℃）。day=0の場合はnull",
    )
    temp_max = serializers.IntegerField(
        read_only=True,
        allow_null=True,
        help_text="最高気温（℃）。day=0の場合はnull",
    )
    pop_00_06 = serializers.IntegerField(
        read_only=True,
        allow_null=True,
        help_text="降水確率 0時〜6時（%）。day=0の場合はnull",
    )
    pop_06_12 = serializers.IntegerField(
        read_only=True,
        allow_null=True,
        help_text="降水確率 6時〜12時（%）。day=0の場合はnull",
    )
    pop_12_18 = serializers.IntegerField(
        read_only=True,
        allow_null=True,
        help_text="降水確率 12時〜18時（%）。day=0の場合はnull",
    )
