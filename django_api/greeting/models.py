...
class EveningGreetingConfig(models.Model):
    """夜のあいさつ設定（シングルトン）"""
    ...
    @classmethod
    def get_solo(cls) -> "EveningGreetingConfig | None":
        """唯一の設定インスタンスを取得（存在しなければ None）."""
        try:
            return cls.objects.get(singleton_key=1)
        except cls.DoesNotExist:
...
class WelcomeHomeGreetingConfig(models.Model):
    """おかえりのあいさつ設定（シングルトン）"""
    ...
    @classmethod
    def get_solo(cls) -> "WelcomeHomeGreetingConfig | None":
        """唯一の設定インスタンスを取得（存在しなければ None）."""
        try:
            return cls.objects.get(singleton_key=1)
        except cls.DoesNotExist:
